...
async function main() {
  try {
    ...
    const candidateLabels = Array.from(new Set([...candidates.map(c=>c.label), "node","python","java","docker","terraform","monorepo","generic"]));
    ...
    const chosen = chooseTemplates(mergedObj.merged, 0.50);
    ...
    // attach classifier metadata for transparency
    ...
    console.log("Preview:", { project_type: values.project_type, language: values.language, package_manager: values.package_manager });
  } catch (err) {
